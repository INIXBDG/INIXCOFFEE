...
<script>
    ...
    function renderTable(data, jabatan) {
        ...
        html += '<th scope="col" style="font-size: 14px;text-align: center;" rowspan="2">Total Fee Instruktur (Rp.)</th>';
        html += '<th scope="col" style="font-size: 14px;text-align: center;" rowspan="2">Alat (Rp.)</th>';
        html += '<th scope="col" style="font-size: 14px;text-align: center;" rowspan="2">Fee Instruktur / Hours (Rp.)</th>';
        ...
        Object.keys(groupedData).forEach(function(materi) {
            ...
            group.forEach(function(item, index) {
                ...
                html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.total_harga_jual)}</td>`;

                if (!item.analisisrkm) {
                    html += '<td style="font-size: 14px;text-align: center;" colspan="13">Belum Diinput data</td>';
                } else {
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.harga_modul_regular ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${item.analisisrkm.harga_modul_regular_dollar ?? 0}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.biaya_modul_regular_dollar ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.biaya_modul_regular ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.konsumsi ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.souvenir ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.pa_hotel ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.exam ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.pc ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.total_fee_instruktur ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.alat ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.fee_instruktur ?? 0)}</td>`;
                    html += `<td style="font-size: 14px;">${formatWithoutDecimals(item.analisisrkm.nett_penjualan ?? 0)}</td>`;
                }
                ...
                html += '<div class="dropdown-menu">';
                    if (item.status == 'Merah') {
                            html += `<a class="dropdown-item" href="/analisisrkm/${item.id}/create" data-toggle="tooltip" title="Input Data"><img src="{{ asset('icon/clipboard-primary.svg') }}" class=""> Input Data</a>`;
                    } 
                    ...
                    // } 
                            ...
                            html += `<a class="dropdown-item disabled" href="/analisisrkm/${item.id}/create" data-toggle="tooltip" title="Input Data"><img src="{{ asset('icon/clipboard-primary.svg') }}" class=""> Input Data</a>`;
                            html += `<a class="dropdown-item" href="/kelasanalisis/${item.id}/edit" data-toggle="tooltip" title="Edit Data"><img src="{{ asset('icon/edit-warning.svg') }}" class=""> Edit Data</a>`;
                    }
                html += '</div>';
                html += '</div>';
                html += '</td>';
            ...
            html += '<tr>';
            html += '<td colspan="18" style="font-size: 14px; text-align: right; font-weight: bold;">Subtotal:</td>';
