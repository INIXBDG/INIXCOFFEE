    ...
    function validateForm(form) {
        ...
        Object.entries(radioGroups).forEach(([group, radios]) => {
            ...
            if (isRequired && !radios.some(r => r.checked)) {
                ...
                (label || first).scrollIntoView({ behavior: 'smooth', block: 'center' });
            } else {
                radios.forEach(r => {
        ...
        form.querySelectorAll('input[data-required]:not([type="radio"]):not([type="checkbox"]), select[data-required], textarea[data-required]').forEach(el => {
            if (!el.value.trim()) {
                isValid = false;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (select) {
            select.addEventListener('change', function() {
                ...
                document.querySelectorAll('.corporate-btn-group button').forEach(btn => btn.classList.remove('active'));
            });
        }
