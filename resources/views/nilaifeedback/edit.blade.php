        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <h5 class="card-title text-center mb-4">{{ __('Pertanyaan Baru') }}</h5>
                    <form method="POST" action="{{ route('nilaifeedback.update', $nilaifeedback->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="row" id="pertanyaan" style="height: 50vh; overflow-y:scroll;">
                            <div class="row">
                                ...
                                @foreach ($materi as $data)
                                    ...
                                    <div class="col-lg-6">
                                        <div class="d-flex justify-content-between mb-2">
                                            @foreach (['1' => 'Kurang', '2' => 'Cukup', '3' => 'Baik', '4' => 'Baik Sekali'] as $value => $label)
                                                <div class="form-check">
                                                    <input type="radio" class="form-check-input" value="{{ $value }}" 
                                                        id="{{ $data->key }}{{ $loop->parent->iteration }}-{{ $value }}" 
                                                        name="{{ $data->key }}{{ $loop->parent->iteration }}"
                                                        {{ ($nilaifeedback[$data->key . $loop->parent->iteration] == $value) ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="{{ $data->key }}{{ $loop->parent->iteration }}-{{ $value }}">{{ $label }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                        <hr class="mt-4">
                                            ...
                                            @foreach (['1' => 'Kurang', '2' => 'Cukup', '3' => 'Baik', '4' => 'Baik Sekali'] as $value => $label)
                                                <div class="form-check">
                                                    <input type="radio" class="form-check-input" value="{{ $value }}" 
                                                        id="{{ $data->key }}{{ $loop->parent->iteration }}-{{ $value }}" 
                                                        name="{{ $data->key }}{{ $loop->parent->iteration }}"
                                                        {{ ($nilaifeedback[$data->key . $loop->parent->iteration . 'b'] == $value) ? 'checked' : '' }}>
                                                    <label class="form-check-label" for="{{ $data->key }}{{ $loop->parent->iteration }}-{{ $value }}">{{ $label }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                            ...
                            <div class="row">
                                ...
                                @foreach ($umum as $data)
                                    ...
                                    <div class="col-lg-6">
                                        <div class="d-flex justify-content-between mb-2">
                                            <textarea class="form-control" name="{{ $data->key }}{{ $loop->iteration }}" 
                                                    id="{{ $data->key }}{{ $loop->iteration }}" cols="12" rows="2">
                                                {{ $nilaifeedback[$data->key . $loop->iteration] ?? '' }}
                                            </textarea>
                                        </div>
                                        <hr class="mt-4">
                                    </div>
