                    ...
                    <table class="table table-striped" id="outstandinghutangTable">  
                        <thead>  
                            <tr>  
                                ...
                                <th scope="col" colspan="8" class="text-center">Tracking</th> <!-- Changed to colspan for tracking -->  
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#outstandinglunasTable').DataTable({
            ...
            "order": [[8, 'desc']], // Ubah urutan menjadi descending untuk kolom ke-6
            "columnDefs": [{"targets": [8], "type": "date"}],
            "footerCallback": function (row, data, start, end, display) {
                ...
                api.column(3, { page: 'current' }).data().each(function (value, index) {
                    var numericValue = parseFloat(value.net_sales) || 0; // Remove non-numeric characters
                    total += numericValue;
                ...
                $('#totalNetSalesThisWeek').html(formatRupiah(totalThisWeek, 'Rp. ')); // Total net sales this week
            },
            "createdRow": function (row, data, dataIndex) {
                ...
                if (status_pembayaran == '1') {
                    if (tanggal_bayar) {
                        var oneWeekAgo = moment().subtract(7, 'days'); // Tanggal satu minggu yang lalu
                        ...
                        if (bayarDate.isAfter(oneWeekAgo)) {
                            ...
                            $(row).css('background-color', '#000B58'); // Background color blue untuk pembayaran lainnya
                            $(row).css('color', 'white'); // Optional: Ubah warna teks menjadi putih
                        }
                    } else {
                        $(row).css('background-color', '#000B58'); // Default jika tanggal_bayar tidak ada
                        $(row).css('color', 'white'); // Optional: Ubah warna teks menjadi putih
                    }
                }
            }
        ...
        $('#outstandinghutangTable').DataTable({  
            ...
            "columns": [  
                ...
                {  
                    ...
                    "render": function(data) {  
                        moment.locale('id');  
                        if (data && data.rkm && data.rkm.tanggal_awal && data.rkm.tanggal_akhir) {  
                            return moment(data.rkm.tanggal_awal).format('LL') + " s/d " + moment(data.rkm.tanggal_akhir).format('LL');  
                        }  
                        return "-";  
                    }  
                },  
                ...
                {  
                    "data": null,  
                    "render": function(data) {  
                        if ( data == null || data.tracking_outstanding == null || data.tracking_outstanding.status_resi == null ) {
                            return "-"; 
                        } else{
                ...
                {  
                    "data": null,  
                    "render": function(data) {  
                        if ( data == null || data.tracking_outstanding == null || data.tracking_outstanding.status_pic == null ) {
                            return "-"; 
                        } else{
            ...
            "footerCallback": function(row, data, start, end, display) {  
                var api = this.api();  
                var total = api.column(3, { page: 'current' }).data().reduce((sum, value) => sum + (parseFloat(value.net_sales) || 0), 0);  
