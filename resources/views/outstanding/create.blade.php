...
@section('content')
                ...
                <div class="card">
                    <div class="card-body" id="card">
                        ...
                        <form method="POST" action="{{ route('outstanding.store') }}" id="formOutstanding">
                            ...
                            <div class="row mb-3">
                                <label for="bulan"
                                    class="col-md-4 col-form-label text-md-start">{{ __('Pilih Bulan dan Tahun') }}</label>
                                <div class="col-md-3">
                                ...
                                <div class="col-md-2">
                                    <button type="button" onclick="getDataRKM()" class="btn click-primary">Cari
                                        Data</button>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="text" class="form-control @error('net_sales') is-invalid @enderror"
                                            name="net_sales" id="net_sales" placeholder="Net Sales" required>
                                    </div>
                                    @error('net_sales')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-4">
                                    <a href="#" target="_blank" id="btnregform"
                                        class="btn btn-md btn-primary d-none">Registrasi Form</a>
                                </div>
    ...
    <script>
        ...
        function toggleTanggalBayar() {
            ...
            if (selectedValue === "1") {
                $('#tanggal_bayar').closest('.row').show();
            } else {
                $('#tanggal_bayar').closest('.row').hide();
            }
        }
        ...
        function statusTracking() {
            ...
            if (selectedValue === "no_resi") {
                $('#status_resi').closest('.row').show();
            } else {
                $('#status_resi').closest('.row').hide();
            }
            if (selectedValue === "konfir_pic") {
                $('#status_pic').closest('.row').show();
                $('#status_resi').closest('.row').show();
            } else {
                $('#status_pic').closest('.row').hide();
            }
        }
        ...
        function getDataRKM() {
            ...
            $.ajax({
                url: "{{ route('getOutstandingRKM', ['year' => 'TANGGAL_TAHUN', 'month' => 'TANGGAL_BULAN']) }}"
                    .replace('TANGGAL_TAHUN', tahun).replace('TANGGAL_BULAN', bulan),
                method: 'GET',
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    if (response.data && response.data.length > 0) {
                        response.data.forEach(function(item) {
                            ...
                            html +=
                                `<option value="${item.id}" data-materi="${item.materi.nama_materi}" data-perusahaan="${item.perusahaan.nama_perusahaan}" data-pax="${item.pax}" data-totalnet="${item.total_net_sales}" data-regform="${registrasiText}" data-saleskey="${item.sales_key}" data-pic="${item.cp}">${item.materi.nama_materi} - ${item.perusahaan.nama_perusahaan}</option>`;
                        });
                    } else {
