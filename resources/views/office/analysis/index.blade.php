    ...
    <div class="container-fluid py-4">
        ...
        @forelse ($years as $year)
            ...
            <div class="mb-5">
                <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
                    <div
                        class="card-header bg-white border-bottom p-4 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                        ...
                        <button class="btn btn-sm btn-outline-info editYearDescBtn" data-year="{{ $year }}"
                            data-desc="{{ $currentYearData?->description ?? '' }}"
                            data-note="{{ $currentYearData?->note ?? '' }}" data-bs-toggle="modal"
                    ...
                    <div class="card-body p-4 bg-light rounded-bottom-4">
                        ...
                        @if (!empty($currentYearData?->description))
                            <div
                                class="alert border border-info rounded-3 mb-4 bg-white shadow-sm d-flex gap-3 align-items-start">
                        ...
                        <div class="row g-4">
                            @foreach ($allMonths as $monthKey => $monthName)
                                @php
                                    ...
                                    $isMonthInQuarterFilter =
                                        empty($selectedQuarter) || in_array($monthKey, $quarterMap[$selectedQuarter]);
                                @endphp
                                            ...
                                            <div class="card-body p-3">
                                                ...
                                                @forelse ($monthReports as $item)
                                                    <div class="p-3 mb-3 border rounded-3 bg-light">
                                                        <div class="mb-2">
                                                            <small class="text-muted d-block mb-1">Oleh:
                                                                {{ $item->user->karyawan->nama_lengkap ?? 'Unknown' }}</small>
                                                        ...
                                                        <div class="d-flex flex-wrap gap-2 mb-3">
                                                            @if (is_array($item->file_paths) && count($item->file_paths) > 0)
                                                                @foreach ($item->file_paths as $index => $path)
                                                                    <a href="{{ route('download.analysis', ['id' => $item->id, 'index' => $index]) }}"
                                                                        target="_blank"
                                                        ...
                                                        <div class="d-flex flex-wrap gap-2 mb-3">
                                                            <p class="mb-2 text-dark mt-2">
                                                                capaian nilai : 
                                                                Rp {{ isset($item->nilai) ? number_format($item->nilai, 0, ',', '.') : '-' }}
                                                            </p>
                                                        </div>
                                                        <div class="d-flex justify-content-end gap-2 border-top pt-2 mt-2">
                                                            <button type="button" class="btn btn-sm btn-warning editBtn"
                                                                ...
                                                                data-month="{{ $item->month }}" data-bs-toggle="modal"
                                                                data-bs-target="#editModal">Edit</button>
                                                            <form action="{{ route('destroy.analysis', $item->id) }}"
                                                                method="POST" class="d-inline">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="btn btn-sm btn-danger"
                                                                    onclick="return confirm('Yakin menghapus data ini?')">Hapus</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                @empty
                ...
                <div class="card border-0 shadow-sm rounded-4 mt-4 mb-5 bg-white">
                    <div class="card-header bg-white border-bottom p-3 d-flex justify-content-between align-items-center">
                        <h6 class="fw-bold text-dark mb-0"><i class="bi bi-pie-chart-fill me-2"></i>Statistik Laporan
                            Tahunan {{ $year }}</h6>
                        ...
                        <div class="row align-items-start mb-0 p-4 border rounded-4 bg-light shadow-sm">
                            ...
                            <div class="col-12 col-md-4 d-flex justify-content-center mb-4 mb-md-0 position-relative">
                                ...
                                <div class="position-absolute top-50 start-50 translate-middle text-center"
                                    ...
                                    <span
                                        class="fs-4 fw-bold text-dark">{{ round(($filledMonthsCount / 12) * 100) }}%</span>
                                    <span class="d-block text-muted"
                                        style="font-size: 0.75rem; margin-top: -5px;">Selesai</span>
                                </div>
                            </div>
                            ...
                            <div class="col-12 col-md-8 ps-md-4">
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-12">
                                        <div
                                            class="d-flex flex-column p-3 rounded-3 bg-white border border-start border-4 border-secondary shadow-sm">
                                            <span class="text-muted fw-semibold small mb-1 text-uppercase">Total
                                                Target</span>
                                    ...
                                    <div class="col-sm-6">
                                        <div
                                            class="d-flex flex-column p-3 rounded-3 bg-white border border-start border-4 border-primary shadow-sm">
                                            <span class="text-muted fw-semibold small mb-1 text-uppercase">Data
                                                Terisi</span>
                                            <span class="fw-bold text-primary fs-5">{{ $filledMonthsCount }}
                                                Laporan</span>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div
                                            class="d-flex flex-column p-3 rounded-3 bg-white border border-start border-4 border-danger shadow-sm">
                                            <span class="text-muted fw-semibold small mb-1 text-uppercase">Belum
                                                Terisi</span>
                                            <span class="fw-bold text-danger fs-5">{{ 12 - $filledMonthsCount }}
                                                Laporan</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="p-3 bg-white border border-info rounded-3 shadow-sm border-start border-4">
                                    <h6 class="fw-bold text-dark mb-2"><i
                                            class="bi bi-info-circle text-info me-2"></i>Catatan Statistik</h6>
                                    <p class="mb-0 text-dark small">
                                        @if (!empty($currentYearData?->note))
                                            {!! nl2br(e($currentYearData->note)) !!}
                                        @else
                                            <span class="text-muted fst-italic">Belum ada catatan untuk statistik tahun
                                                ini.</span>
                                        @endif
                                    </p>
                                </div>
                                        ...
                                        <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
                                            <div
                                                ...
                                                <button class="btn btn-sm btn-outline-info editQuarterDescBtn"
                                                    data-year="{{ $year }}" data-quarter="{{ $qKey }}"
                                                    data-desc="{{ $qData->description ?? '' }}"
                                                    data-format="{{ $qData->format_nilai ?? '' }}"
                                                    data-nilai="{{ $qData->nilai ?? '' }}" data-bs-toggle="modal"
                                            ...
                                            <div class="card-body p-4 bg-white rounded-bottom-4">
                                                ...
                                                @if (!empty($qData->nilai))
                                                    <div
                                                        class="mb-3 p-3 bg-light rounded-3 border-start border-4 border-info shadow-sm d-flex justify-content-between align-items-center">
                                                        <span class="fw-semibold text-muted small text-uppercase">Capaian
                                                            Nilai</span>
                                                ...
                                                @else
                                                    <p class="text-muted small fst-italic mb-3">Deskripsi triwulan belum
                                                        diatur.</p>
                                                @endif

                                                <div class="border-top pt-3">
                                                    <h6 class="small fw-bold text-secondary mb-2">Lampiran File Triwulan:
                                                    </h6>
                                                    <div class="d-flex flex-wrap gap-2">
                                                        @if (!empty($qData->file_paths) && count($qData->file_paths) > 0)
                                                            @foreach ($qData->file_paths as $index => $path)
                                                                <a href="{{ route('download.quarter.analysis', ['year' => $year, 'quarter' => $qKey, 'index' => $index]) }}"
                                                                    target="_blank"
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
                    <div class="card-header bg-white border-bottom p-4">
                        <h5 class="fw-bold text-dark mb-0 border-start border-4 border-info ps-3">Analisis Tahun
                            {{ $year }}</h5>
                    </div>
                    <div class="card-body p-4 bg-light rounded-bottom-4">
                        @php
                        ...
                        <div class="card border-0 shadow-sm rounded-4 bg-white">
                            <div
                                ...
                                <button class="btn btn-sm btn-info text-white editAnnualBtn"
                                    data-year="{{ $year }}" data-desc="{{ $aData->description ?? '' }}"
                                    data-bs-toggle="modal" data-bs-target="#annualModal">
                                    ...
                                    <div class="d-flex flex-wrap gap-2">
                                        @if (!empty($aData->file_paths) && count($aData->file_paths) > 0)
                                            @foreach ($aData->file_paths as $index => $path)
                                                <a href="{{ route('download.annual.analysis', ['year' => $year, 'index' => $index]) }}"
        ...
        <div class="modal fade" id="createModal" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form action="{{ route('store.analysis') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="modal-body pt-3">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nilai (Rupiah)</label>
                            <input type="text" id="nilaiDisplay" class="form-control" placeholder="contoh : 200.000.000"
                                required>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <form id="editForm" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="modal-body pt-3">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Nilai (Rupiah)</label>
                            <input type="number" name="nilai" class="form-control edit_nilai" id="nilai">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Deskripsi</label>
        ...
        <div class="modal fade" id="yearDescModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form action="{{ route('update.year.description.analysis') }}" method="POST"
                    class="modal-content shadow-lg border-0 rounded-4">
                    @csrf
                    <div class="modal-header border-0 pb-0">
                    ...
                    <div class="modal-body pt-3">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Tahun</label>
                            <input type="number" id="desc_year" name="year" class="form-control bg-light" readonly
                                required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Deskripsi</label>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="note_text" name="note" class="form-control" rows="4"
                                placeholder="Masukkan catatan atau keterangan persentase..."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer border-0">
                        <button type="button" class="btn btn-secondary px-4" data-bs-dismiss="modal">Batal</button>
        ...
        <div class="modal fade" id="quarterDescModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form action="{{ route('update.quarter.description.analysis') }}" method="POST"
                    enctype="multipart/form-data" class="modal-content shadow-lg border-0 rounded-4">
                    @csrf
                    <div class="modal-header border-0 pb-0">
                    ...
                    <div class="modal-body pt-3">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Tahun</label>
                                <input type="number" id="desc_quarter_year" name="year" class="form-control bg-light"
                                    readonly required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Triwulan</label>
                                <input type="number" id="desc_quarter_val" name="quarter" class="form-control bg-light"
                                    readonly required>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" id="desc_quarter_nilai" name="nilai"
                                    class="form-control" placeholder="Contoh: 85.50">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="files[]" class="form-control" multiple>
                            <small class="text-danger mt-1 d-block">*Mengunggah file baru akan menimpa seluruh file triwulan
                                sebelumnya.</small>
                        </div>
                    </div>
                    <div class="modal-footer border-0">
        ...
        <div class="modal fade" id="annualModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <form action="{{ route('update.annual.description.analysis') }}" method="POST"
                    enctype="multipart/form-data" class="modal-content shadow-lg border-0 rounded-4">
                    @csrf
                    <div class="modal-header border-0 pb-0">
                    ...
                    <div class="modal-body pt-3">
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Tahun</label>
                            <input type="number" id="annual_year" name="year" class="form-control bg-light" readonly
                                required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="files[]" class="form-control" multiple>
                            <small class="text-danger mt-1 d-block small">*Unggahan baru akan mengganti file
                                sebelumnya.</small>
                        </div>
                    </div>
                    <div class="modal-footer border-0">
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                @forelse ($years as $year)
                    ...
                    const ctx_{{ $year }} = document.getElementById('doughnutChart_{{ $year }}')
                        .getContext('2d');
