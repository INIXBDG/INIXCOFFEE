...
@section('crm_contents')
                    ...
                    <div class="table-responsive rounded-3 border">
                        <table class="table table-hover align-middle mb-0 shadow-none">
                            <thead class="bg-light">
                                <tr>
                                    <th class="small border-0">No</th>
                                    <th class="small border-0 text-center">Waktu</th>
                                    <th class="small border-0">Client</th>
                                    <th class="small border-0">Deskripsi</th>
                                    <th class="small border-0">Nilai</th>
                                </tr>
                            </thead>
                            <!-- Penambahan ID bodyDetailAktivitas dilakukan di sini -->
                            <tbody id="bodyDetailAktivitas" class="small">
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
            ...
            window.loadWinLostCharts = function(tahun) {
                fetch(`/crm/total-win-lost?tahun=${tahun}`)
                    ...
                    .then(dataApi => {
                        ...
                        if (chartWinInstance) chartWinInstance.destroy();
                        ...
                        if (chartLostInstance) chartLostInstance.destroy();
        ...
        // IMPLEMENTASI FUNGSI SHOW ACTIVITY DETAILS
        window.showActivityDetails = async function(salesId, activityLabel) {
            const sales = activityData.find(s => s.id_sales === salesId);

            if (!sales) {
                console.error("Kesalahan: Entitas Sales tidak ditemukan untuk ID:", salesId);
                alert("Data referensi sales tidak ditemukan.");
                return;
            }
            ...
            const selectedActivity = activityMap[activityLabel];

            if (!selectedActivity) {
                console.error("Kesalahan: Label aktivitas tidak valid:", activityLabel);
                return;
            }
            ...
            let url = `/crm/detail-aktivitas-api?id_sales=${encodeURIComponent(salesId)}&aktivitas=${encodeURIComponent(activityLabel)}`;

            if (start && end) {
                url += `&start_date=${encodeURIComponent(start)}&end_date=${encodeURIComponent(end)}`;
            }

            // Deklarasi Variabel DOM Sesuai HTML Aktual
            const modalElement = document.getElementById('detailAktivitas');
            const tableBody = document.getElementById('bodyDetailAktivitas');

            if (modalElement) {
                // Injeksi Data ke DOM Header Modal
                document.getElementById('modalActivity').innerText = activityLabel;
                document.getElementById('modalSalesId').innerText = salesId;

                // Kalkulasi dan Injeksi Progress Capaian
                const jumlah = selectedActivity.jumlah || 0;
                const target = selectedActivity.target || 0;
                ...
                document.getElementById('modalPersen').innerText = persen + '%';
                document.getElementById('modalProgressBar').style.width = persen + '%';

                if (tableBody) tableBody.innerHTML = '<tr><td colspan="5" class="text-center">Memuat data aktivitas...</td></tr>';

                // Eksekusi Tampilan Modal Kustom
                ...
                fetch(url)
                    ...
                    .then(data => {
                        if (tableBody) {
                            tableBody.innerHTML = '';
                            if (data.length === 0) {
                                tableBody.innerHTML = '<tr><td colspan="5" class="text-center">Tidak ada rincian aktivitas.</td></tr>';
                                return;
                            }
                            data.forEach((item, index) => {
                                const namaPerusahaan = item.contact?.perusahaan?.nama_perusahaan || '-';
                                const tanggal = new Date(item.waktu_aktivitas).toLocaleDateString('id-ID');
                                tableBody.innerHTML += `
                                    <tr>
                                        <td>${index + 1}</td>
                                        ...
                                        <td>${new Intl.NumberFormat('id-ID').format(item.total || 0)}</td>
                                    </tr>
                                `;
                            });
                        }
                    ...
                    .catch(error => {
                        console.error("Kesalahan pemuatan rincian:", error);
                        if (tableBody) tableBody.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Gagal memuat rincian.</td></tr>';
                    });
            } else {
                console.error("Peringatan: Elemen DOM dengan ID 'detailAktivitas' tidak ditemukan pada kerangka View.");
            }
        };
