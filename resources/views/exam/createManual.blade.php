        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-body" id="card">
                    ...
                    <h5 class="card-title text-center mb-4">{{ __('Tambah Pengajuan Exam Manual') }}</h5>
                    <form method="POST" action="{{ route('exam.store') }}">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select class="form-control @error('rkm') is-invalid @enderror" name="id_rkm" id="rkm" required>
                                    <option value="" hidden disabled selected>-- Pilih RKM --</option>
                                    @foreach ($rkm as $r)
                                        <option class="form-control" value="{{ $r->id }}" data-id="{{ $r->id }}" data-materi="{{ $r->materi->nama_materi }}" data-perusahaan="{{ $r->perusahaan->nama_perusahaan }}">{{ $r->materi->nama_materi }} || {{ $r->perusahaan->nama_perusahaan }} || {{ \Carbon\Carbon::parse($r->tanggal_awal)->format('d M Y') }}-{{ \Carbon\Carbon::parse($r->tanggal_akhir)->format('d M Y') }}</option>
                                    @endforeach
                                </select>
                                @error('rkm')
                                    <span class="invalid-feedback" role="alert">
                        ...
                        {{-- hidden input --}}
                        <input type="text" hidden name="perusahaan" id="perusahaan">
                        <input type="text" hidden name="materi" id="materi">
...
$(document).ready(function() {
    ...
    // set hidden input 
    $('#rkm').on('change', function () {
        let selected = $(this).find(':selected');

        let perusahaan = selected.data('perusahaan');
        let materi = selected.data('materi');
