                ...
                <div class="stat-card h-100">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 text-primary" id="rataProgress">0%</h4>
                        </div>
                ...
                <div class="stat-card h-100">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 text-warning" id="kpiAktif">0</h4>
                        </div>
                ...
                <div class="stat-card h-100">
                    <div class="card-body d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 text-success" id="kpiSelesai">0</h4>
                        </div>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                    ...
                    <div class="btn-group filter-btn-group">
                        <button class="btn active" onclick="filterTargets('all')">Semua</button>
                        <button class="btn" onclick="filterTargets('active')">Aktif</button>
                        <button class="btn" onclick="filterTargets('completed')">Selesai</button>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateCharts(statistikTargets, distribusiStatus) {
            ...
            let borderColors = statistikTargets.map(item => {
                let progress = item.progress || 0;
                if (progress === 0) return 'rgba(254, 215, 24, 1)';
                if (item.tipe_target === "rupiah") {
                    let percent = item.target ? (progress / item.target) * 100 : 0;
                    return percent >= 100 ? 'rgba(40, 207, 180, 1)' : 'rgba(254, 124, 150, 1)';
                }
                return progress >= item.target ? 'rgba(40, 207, 180, 1)' : 'rgba(254, 124, 150, 1)';
            });
        ...
        function updateTargetCards(targets) {
            ...
            targets.forEach((target, index) => {
                ...
                let cardBorderColor = '';
                if (target.progress >= 75) cardBorderColor = 'border-success';
                else if (target.progress >= 50) cardBorderColor = 'border-primary';
                else cardBorderColor = 'border-warning';
                ...
                let targetChange;
                if (target.tipe_target === "rupiah") targetChange = "Rp " + Number(target.target).toLocaleString(
                    "id-ID");
                else if (target.tipe_target === "persen") targetChange = target.target + "%";
                else targetChange = target.target;
                        ...
                        <div class="target-card border-start border-4 ${cardBorderColor}">
                            ...
                            <button type="button" class="btn btn-link p-0 text-start w-100 h-100 text-decoration-none buttonDetailTarget" data-id="${target.id}" data-bs-toggle="modal" data-bs-target="#detailTargetModal" style="outline: none;">
                                <div class="card-body-inner">
                                    ...
                                    <h6 class="fw-bold text-dark mb-2 lh-base title-badge">
                                        ${target.judul}
                                    </h6>
        ...
        function filterTargets(filter) {
            ...
            if (filter === 'active') filteredTargets = allTargetsData.filter(target => target.status === 'Aktif');
            else if (filter === 'completed') filteredTargets = allTargetsData.filter(target => target.status === 'Selesai');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (!data) {
                        ...
                        let bgCard;
                        if (Tercapai === "Mencapai Target") bgCard = "success";
                        else if (Tercapai === "Target Gagal") bgCard = "danger";
                        else if (Tercapai === "Sedang Berjalan") bgCard = "warning";
                        else bgCard = "secondary";
                        ...
                        const allowedAssistantRoutesForTargetPenjualanTahunan = [
                            'target penjualan tahunan', 'Pemasukan Kotor'
                        ];
                        ...
                        if (allowedAssistantRoutesForTargetPenjualanTahunan.includes(data.condition)) {
                            ...
                            if (salesPerf && salesPerf.data) {
                                const formatRupiah = (num) => 'Rp ' + Number(num).toLocaleString(
                                    'id-ID');
                                if (salesPerf.type === 'individual') {
                                    const s = salesPerf.data;
                                    const statusClass = s.status === 'achieved' ? 'badge-success' :
                                        'badge-warning';
                                    const progressColor = s.status === 'achieved' ? '#28a745' :
                                        '#ffc107';
                                    ...
                                    salesPerf.data.forEach((sales, index) => {
                                        const statusClass = sales.status === 'achieved' ?
                                            'badge-success' : 'badge-warning';
                                        const textClass = sales.status === 'achieved' ?
                                            'text-success' : 'text-warning';
                                        const targetValue = Number(sales.presentase_kemampuan)
                                            .toLocaleString('id-ID', {
                                                useGrouping: false
                                            });
                                        rows += `
                                    ...
                                    Object.entries(data.data_detail.triwulan_data).forEach(([label,
                                        value
                                    ]) => {
                                        htmlTargetTahunanSales +=
                                            `<div class="col-md-6"><div class="card h-100 shadow-sm border-0"><div class="card-body"><h5 class="card-title">${label.replace('_', ' ')}</h5><p class="card-text">Rp ${value.toLocaleString('id-ID')}</p></div></div></div>`;
                                    });
                                    ContentTrafikSales = `
                                        <div class="row">
                                            <div class="col"><div class="card shadow-sm mt-3"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0"><thead class="thead-light"><tr><th class="text-center" width="5%">No</th><th>Sales</th><th class="text-right" width="20%">Revenue</th><th class="text-right" width="20%">Target (Editable)</th><th class="text-center" width="15%">Persentase</th><th class="text-center" width="15%">Status</th></tr></thead><tbody>${rows}</tbody></table></div></div></div></div>
                                    ...
                                    `;
                                    setTimeout(() => {
                                        document.querySelectorAll('.target-input').forEach(
                                            input => {
                                                let timeout = null;
                                                            ...
                                                            success: function(
                                                                ...
                                                                if (response
                                                                    ...
                                                                    if (response
                                                                        ...
                                                                        percentageCell
                                                                        .innerHTML =
                                                                        `<strong class="${response.data.status === 'achieved' ? 'text-success' : 'text-warning'}">${response.data.percentage}%</strong>`;
                                                                    if (response
                                                                        .data
                                                                        .status
                                                                        ) {
                                                                        ...
                                                                        statusCell
                                                                            .innerHTML =
                                                                            `<span class="badge ${statusClass}">${response.data.status.toUpperCase()}</span>`;
                                                                    }
                                                                }
                                                                setTimeout
                                                                    (() => {
                        ...
                        } else if (allowedAssistantRoutesForPeningkatanKontribusiPelatihan.includes(data
                                .condition)) {
                            ContentTrafikSales = `
                        ...
                        if (allowedDetailAssistantRoutes.includes(data.condition)) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                fileContent =
                                    `<div class="w-100 h-100 d-flex flex-column align-items-center justify-content-center p-3"><div class="mb-3" style="max-width: 100%; max-height: 300px;"><img src="${fullFileUrl}" alt="${fileName}" class="img-fluid rounded shadow-sm" style="max-width: 100%; max-height: 300px; object-fit: contain;"></div><div class="mt-2 text-center"><a href="${fullFileUrl}" download="${fileName}" class="btn btn-primary btn-sm"><i class="fa-solid fa-download me-1"></i>Download Gambar</a></div><div class="mt-2 small text-muted"><i class="fa-solid fa-file-image me-1"></i>${fileName}</div></div>`;
                            } else if (pdfExtensions.includes(fileExtension)) {
                                fileContent =
                                    `<div class="w-100 h-100 d-flex flex-column"><div class="flex-grow-1 mb-3" style="min-height: 250px;"><iframe src="${fullFileUrl}" class="w-100 h-100" style="border: 1px solid #dee2e6; border-radius: 8px;"></iframe></div><div class="text-center"><a href="${fullFileUrl}" download="${fileName}" class="btn btn-primary btn-sm"><i class="fa-solid fa-download me-1"></i>Download PDF</a></div><div class="mt-2 small text-muted text-center"><i class="fa-solid fa-file-pdf me-1"></i>${fileName}</div></div>`;
                            } else {
                                fileContent =
                                    `<div class="text-center py-5"><div class="mb-3"><i class="fa-solid fa-file text-secondary" style="font-size: 4rem;"></i></div><p class="text-muted mb-3">File tidak dapat ditampilkan</p><p class="text-muted small">Hanya gambar dan PDF yang dapat ditampilkan</p></div>`;
                            }
                            contentPieChart =
                                `<h6 class="fw-semibold mb-3 text-secondary"><i class="fa-solid fa-file me-2"></i>Dokumen Manual</h6><div class="manual-document-container flex-grow-1 d-flex flex-column align-items-center justify-content-center p-3" style="background-color: #f8f9fa; border-radius: 8px;">${fileContent}</div><div class="mt-3 small text-muted text-center"><i class="fa-solid fa-info-circle me-1"></i>Klik tombol download untuk menyimpan file</div>`;
                        } else if (allowedDetailAssistantRoutesForRupiah.includes(data.condition)) {
                            contentPieChart =
                                `<div class="mb-4"><h6 class="fw-semibold text-primary mb-1"><i class="fa-solid fa-wallet me-2"></i>${data.judul}</h6><small class="text-muted">Ringkasan performa</small></div><div class="mb-4 p-3 rounded bg-light"><div class="text-muted small mb-1">Bulan Terakhir</div><div class="fw-semibold">${labelBulanTerakhir}</div><div class="fw-bold fs-6 text-dark">${formatRupiah(nilaiBulanTerakhirRupiah)}</div></div><div class="mb-3"><div class="text-muted small mb-2">Top Hari Tertinggi</div>${top3HariTertinggi.map(([tanggal, nilai], index) => `<div class="d-flex justify-content-between mb-1 ${index > 0 ? 'text-muted' : ''}"><span>${formatTanggalSingkat(tanggal)}</span><span class="fw-semibold">${formatRupiah(nilai)}</span></div>`).join('')}</div><hr class="my-3"><div class="d-flex align-items-center gap-2 text-muted small"><i class="bi bi-person-circle fs-5"></i><span class="fw-semibold text-dark">${karyawanTerkaitRupiah?.nama_lengkap ?? '-'}</span><span>• ${karyawanTerkaitRupiah?.jabatan ?? '-'}</span></div>`;
                        } else {
                            contentPieChart =
                                `<h6 class="fw-semibold mb-3 text-secondary"><i class="fa-solid fa-chart-pie me-2"></i>Chart ${data.condition}</h6><div class="chart-container flex-grow-1"><canvas id="MyChartDoughtnut"></canvas></div>`;
                        }

                        let contentStatisticChart = '';
                        if (allowedDetailAssistantRoutes.includes(data.condition)) {
                            contentStatisticChart = ``;
                        } else if (allowedAssistantRoutesForLaporanAnalisisKeuangan.includes(data
                                .condition)) {
                        ...
                        } else if (allowedDetailAssistantRoutesForPresentaseGapKompetensi.includes(data
                                .condition)) {
                            contentStatisticChart = `
                                <div class="mt-4"><div class="card shadow-sm border-0 rounded-4"><div class="card-body"><h6 class="fw-semibold mb-3">Input Presentase Kemampuan Programmer</h6>
                                    <form id="formGapKompetensi">
                                        ...
                                        @if ($allowed) <div class="mt-3"><button type="submit" class="btn btn-primary">Simpan</button></div> @endif
                                    </form>
                                </div></div></div>
                            `;
