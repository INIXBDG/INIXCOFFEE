            ...
            $(document).on('click', '.buttonDetailTarget', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        let Tercapai;
                        if (dateNow < startOfYear) {
                            Tercapai = "Belum Dimulai";
                        } else if (dateNow > tenggatWaktu || dateNow === tenggatWaktu) {
                        ...
                        let bgCard = Tercapai === "Mencapai Target" ? "success" :
                                    Tercapai === "Target Gagal" ? "danger" :
                                    Tercapai === "Sedang Berjalan" ? "warning" : "secondary";
                        ...
                        const formatRupiah = (nilai = 0) => 'Rp ' + Number(nilai).toLocaleString('id-ID');
                        ...
                        const labelBulanTerakhir = new Date(`${keyBulanTerakhirRupiah}-01`).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' });
                        const seluruhDataHarian = Object.values(dataHarianRupiah).flatMap(bulan => Object.entries(bulan));
                        const top3HariTertinggi = seluruhDataHarian.filter(([_, nilai]) => nilai > 0).sort((a, b) => b[1] - a[1]).slice(0, 3);
                        ...
                        if (allowedAssistantRoutesForTargetPenjualanTahunan.includes(data.condition)) {
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="mb-2">
                                                            ...
                                                            <div class="progress" style="height: 10px;">
                                                                <div class="progress-bar" role="progressbar" style="width: ${progressWidth}%; background-color: ${progressColor};" aria-valuenow="${s.percentage}" aria-valuemin="0" aria-valuemax="100"></div>
                                                            </div>
                                                        </div>
                                                        <div class="text-right"><span class="badge ${statusClass} p-2">${s.status.toUpperCase()}</span></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>`;
                                            ...
                                            <tr id="row-${sales.kode_karyawan}">
                                                ...
                                                <td class="text-center">
                                                    <div class="input-group input-group-sm" style="max-width: 150px; float: right;">
                                                        <input type="text" class="form-control text-right target-input ${sales.id_detailPerson ? '' : 'is-invalid'}" value="${targetValue}" data-id-detail="${sales.id_detailPerson || ''}" data-kode-karyawan="${sales.kode_karyawan}" placeholder="Target" ${!sales.id_detailPerson ? 'disabled' : ''}>
                                                    </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col">
                                                <div class="card shadow-sm border-0 mt-3">
                                                    <div class="card-body">
                                                        <div class="row g-3">${htmlTargetTahunanSales}</div>
                                                    </div>
                                                    <div class="mb-2 text-center"><hr><p>Data Triwulan diambil dari tahun ${data.detail_jangka}</p></div>
                                                </div>
                                            </div>
                                        </div>`;
                                }
                            ...
                            ContentTrafikSales = `
                                                ...
                                                <div class="card border-0 shadow-sm h-100 quarter-card">
                                                    <div class="card-body d-flex flex-column p-4">
                                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                                            <div><h6 class="fw-semibold text-muted mb-1">Periode</h6><h5 class="fw-bold mb-0">${bulanIndo[item.month - 1] ?? '-'}</h5></div>
                                                            <span class="badge rounded-pill bg-${item.color} bg-opacity-10 text-${item.color} px-3 py-2">Laporan</span>
                                                        </div>
                                                        ...
                                                        <div class="d-flex justify-content-end align-items-center mt-4">
                                                            <a href="{{ asset('${item.file_paths}') }}" class="btn btn-sm btn-dark d-flex align-items-center gap-2" download><i class="fas fa-download"></i> Download</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>`).join('')}
                                ...
                                <style>
                                    .quarter-card { border-radius: 16px; transition: all 0.25s ease; background: #ffffff; }
                                    .quarter-card:hover { transform: translateY(-6px) scale(1.01); box-shadow: 0 15px 35px rgba(0,0,0,0.08); }
                                    .quarter-card h3 { letter-spacing: 0.5px; }
                                    .quarter-card .badge { font-size: 12px; font-weight: 500; }
                                    .description-text { display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
                                    .description-text.expanded { -webkit-line-clamp: unset; overflow: visible; }
                                    .quarter-card .btn { border-radius: 8px; font-size: 13px; }
                                </style>`;
                        } else if (allowedAssistantRoutesForKepuasanPelanggan.includes(data.condition)) {
                            const item = data.data_detail;
                            ContentTrafikSales = `
                                <div class="row g-4 mt-1">
                                    <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">RINGKASAN</h6><div class="d-flex justify-content-between mb-2"><span class="text-muted">Total Feedback</span><span class="fw-bold">${item.total_feedback ?? 0}</span></div><div class="d-flex justify-content-between mb-2"><span class="text-muted">Total Sesi</span><span class="fw-bold">${item.total_sessions ?? 0}</span></div><div class="d-flex justify-content-between"><span class="text-muted">Prediksi</span><span class="fw-bold text-primary">${item.prediction ?? 0}</span></div></div></div></div>
                                    <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">PERFORMA</h6><div class="mb-3"><small class="text-muted d-block">Top Performer</small><span class="fw-bold text-success">${item.top_performer?.label ?? '-'} (${item.top_performer?.value ?? 0})</span></div><div><small class="text-muted d-block">Lowest Performer</small><span class="fw-bold text-danger">${item.lowest_performer?.label ?? '-'} (${item.lowest_performer?.value ?? 0})</span></div></div></div></div>
                                    <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">STATUS</h6><div class="mb-2"><span class="badge bg-${item.trend === 'up' ? 'success' : (item.trend === 'down' ? 'danger' : 'secondary')}">Trend: ${item.trend} (${item.trend_value})</span></div><div class="mb-2"><span class="badge bg-${item.consistency === 'stable' ? 'success' : 'warning'}">Konsistensi: ${item.consistency}</span></div><div><span class="badge bg-${item.target_status === 'on_track' ? 'success' : (item.target_status === 'at_risk' ? 'warning' : 'danger')}">Target: ${item.target_status}</span></div></div></div></div>
                                    <div class="col-12"><div class="card shadow-sm border-0 rounded-4"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">KATEGORI PENILAIAN</h6><div class="row text-center">${Object.entries(item.category_scores || {}).map(([key, val]) => `<div class="col"><div class="p-2 border rounded-3"><small class="text-muted d-block">${key}</small><h5 class="fw-bold mb-0">${val}</h5></div></div>`).join('')}</div></div></div></div>
                                    <div class="col-12"><div class="alert alert-info rounded-4 shadow-sm mb-0"><i class="fa-solid fa-lightbulb me-2"></i>${item.insight ?? '-'}</div></div>
                                </div>`;
                            ...
                            <div class="card mt-4 border-0 rounded-4" style="background:#f8fafc;">
                                <div class="card-body p-4">
                                    <div class="d-flex justify-content-between align-items-end mb-4">
                                        <div><div class="text-muted small">Total Kelas</div><div class="fs-2 fw-semibold text-dark">${data.data_detail.class_breakdown.total}</div></div>
                                    </div>
                                    <div class="row g-3 mb-4">
                                        <div class="col-md-6"><div class="p-3 rounded-3 bg-white border h-100"><div class="text-muted small mb-2">Kelas Inixindo</div><div class="fs-4 fw-semibold text-dark">${data.data_detail.class_breakdown.kelas_od}</div></div></div>
                                        <div class="col-md-6"><div class="p-3 rounded-3 bg-white border h-100"><div class="text-muted small mb-2">Kelas Orang Luar</div><div class="fs-4 fw-semibold text-dark">${data.data_detail.class_breakdown.kelas_ol}</div></div></div>
                                        <div class="col-md-6"><div class="p-3 rounded-3 bg-white border h-100"><div class="text-muted small mb-2">Kelas Offline</div><div class="fs-4 fw-semibold text-dark">${data.data_detail.class_breakdown.kelas_offline}</div></div></div>
                                        <div class="col-md-6"><div class="p-3 rounded-3 bg-white border h-100"><div class="text-muted small mb-2">Kelas Online</div><div class="fs-4 fw-semibold text-dark">${data.data_detail.class_breakdown.kelas_online}</div></div></div>
                                    </div>
                                    <div class="p-3 rounded-3 bg-white border">
                                        <div class="fw-semibold mb-3">Kelas Inhouse</div>
                                        <div class="d-flex justify-content-between py-2 border-bottom"><span class="text-muted small">Bandung</span><span class="fw-semibold text-dark">${data.data_detail.class_breakdown.Inhouse.kelas_inhouse}</span></div>
                                        <div class="d-flex justify-content-between py-2"><span class="text-muted small">Luar Bandung</span><span class="fw-semibold text-dark">${data.data_detail.class_breakdown.Inhouse.kelas_inhouse_luar}</span></div>
                                    </div>
                                </div>
                            </div>`;
                        }
                        ...
                        if (allowedAssistantRoutes.includes(data.condition)) {
                            ...
                            const divisionHtml = Object.entries(item.division_breakdown || {}).map(([div, val]) => `<div class="col mb-3"><div class="p-2 border rounded-3"><small class="text-muted d-block">${div}</small><h5 class="fw-bold mb-0">${val}%</h5></div></div>`).join('');
                            const riskHtml = (item.risk_divisions && item.risk_divisions.length > 0) ? `<div class="col-6"><div class="card shadow-sm border-0 rounded-4"><div class="card-body"><h6 class="fw-semibold text-danger mb-3">DIVISI BERISIKO</h6><div class="row">${item.risk_divisions.map(risk => `<div class="col mb-2"><div class="p-2 border rounded-3 text-center"><small class="text-muted d-block">${risk.name}</small><span class="fw-bold text-danger">${risk.value}%</span></div></div>`).join('')}</div></div></div></div>` : '';
                            contentStatisticChart = `
                            <div class="row g-4 mt-1">
                                <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">RINGKASAN</h6><div class="d-flex justify-content-between mb-2"><span class="text-muted">Total KPI</span><span class="fw-bold">${item.total_kpi ?? 0}</span></div><div class="d-flex justify-content-between mb-2"><span class="text-muted">Total Divisi</span><span class="fw-bold">${item.total_division ?? 0}</span></div><div class="d-flex justify-content-between"><span class="text-muted">Progress</span><span class="fw-bold text-primary">${item.progress ?? 0}%</span></div></div></div></div>
                                <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">PERFORMA DIVISI</h6><div class="mb-3"><small class="text-muted d-block">Top Division</small><span class="fw-bold text-success">${item.top_division?.name ?? '-'} (${item.top_division?.value ?? 0}%)</span></div><div><small class="text-muted d-block">Lowest Division</small><span class="fw-bold text-danger">${item.lowest_division?.name ?? '-'} (${item.lowest_division?.value ?? 0}%)</span></div></div></div></div>
                                <div class="col-lg-4"><div class="card shadow-sm border-0 rounded-4 h-100"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">STATUS</h6><div class="mb-2"><span class="badge bg-${trendColor}">Trend: ${trend} (${trendValue})</span></div><div class="mb-2"><span class="badge bg-${consistencyColor}">Konsistensi: ${consistency}</span></div><div><span class="badge bg-${targetColor}">Target: ${targetStatus}</span></div></div></div></div>
                                <div class="col-6"><div class="card shadow-sm border-0 rounded-4"><div class="card-body"><h6 class="fw-semibold text-secondary mb-3">BREAKDOWN DIVISI</h6><div class="row text-center">${divisionHtml}</div></div></div></div>
                                ${riskHtml}
                                <div class="col-12"><div class="alert alert-info rounded-4 shadow-sm mb-0"><i class="fa-solid fa-lightbulb me-2"></i>${item.insight ?? '-'}</div></div>
                            </div>`;
                        } else if (allowedAssistantRoutesForLaporanAnalisisKeuangan.includes(data.condition)) {
                            const bulanIndo = ['', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
                            contentStatisticChart = `<div class="row g-4">${(data.data_detail.analisa_data).map(item => `<div class="col-md-4 mt-5"><div class="card border-0 shadow-sm h-100"><div class="mb-2 p-3"><h5 class="fw-bold text-primary mb-1">${bulanIndo[item.month]}</h5><small class="text-muted">Laporan Analisis Bulanan</small></div><div class="card-body d-flex flex-column" style="overflow-y: scroll; max-height: 280px;"><div class="mb-3"><p class="mb-0" style="text-align: justify;">${item.description}</p></div></div><div class="mt-auto p-3"><a href="{{ asset('${item.file_paths}') }}" class="btn btn-sm btn-outline-primary w-100"><i class="fas fa-file-alt"></i></a></div></div></div>`).join('')}</div>`;
                                    ...
                                    <div class="card shadow-sm border-0 rounded-4">
                                        <div class="card-body">
                                            ...
                                            <form id="formGapKompetensi">
                                                @php $allowed = auth()->user()->jabatan === 'Koordinator ITSM'; @endphp
                        ...
                        if (data.tipe_target === "rupiah") {
                            widthProgress = Number(((data.data_detail.progress / data.nilai_target) * 100).toFixed(1));
                        } else {
                        ...
                        setTimeout(function() {
                            ...
                            document.querySelectorAll('.target-input').forEach(input => {
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(response) {
                                                ...
                                                if (response.data) {
                                                    ...
                                                    if (response.data.percentage) percentageCell.innerHTML = `<strong class="${response.data.status === 'achieved' ? 'text-success' : 'text-warning'}">${response.data.percentage}%</strong>`;
                                                ...
                                                setTimeout(() => { feedback.style.display = 'none'; el.classList.remove('is-valid'); }, 2000);
                                            },
                                            error: function() {
                                                spinner.style.display = 'none';
                                                el.disabled = false;
                                ...
                                input.addEventListener('blur', function() { const value = this.value.replace(/\./g, ''); if (value) this.value = Number(value).toLocaleString('id-ID'); });
                                input.addEventListener('focus', function() { const value = this.value.replace(/\./g, ''); if (value) this.value = value; });
                            });
                            ...
                            $(document).off('submit', '#formGapKompetensi').on('submit', '#formGapKompetensi', function(e) {
                                ...
                                $.ajax({
                                    ...
                                    data: $(this).serialize(),
                                    success: function() {
                                        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Berhasil diupdate.', timer: 2000, showConfirmButton: false }).then(() => { $('#detailTargetModal').modal('hide'); if (typeof loadContentForm === 'function') loadContentForm(); });
                                    },
                                    error: function(err) {
                            ...
                            const renderStatistic = (labels, values, label) => {
                                ...
                                statisticChart = new Chart(statisticCtx, {
                                    type: 'line',
                                    data: { labels, datasets: [{ label, data: values, borderColor: '#4e73df', backgroundColor: 'rgba(78, 115, 223, 0.1)', tension: 0.4, fill: true }] },
                            ...
                            $('#filterType').off('change').on('change', function() {
                                if (this.value === 'month') {
                                    ...
                                    Object.keys(dailyData).forEach(monthKey => $('#filterMonth').append(`<option value="${monthKey}">${getNamaBulan(monthKey)}</option>`));
                                    if (statisticChart) statisticChart.destroy();
                                } else {
                    ...
                    error: function(err) {
                        Swal.fire('Error', err.responseJSON?.message || 'Gagal memuat detail target', 'error');
                    }
                });
            });
