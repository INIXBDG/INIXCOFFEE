        ...
        <script>
                ...
                Object.entries(groupedByPembuat).forEach(([idPembuat, group]) => {
                    group.targets.forEach(function(item) {
                        ...
                        if (item.tipe_target === 'persen') {
                            ...
                            formattedTarget = isPerPesertaRoute 
                                ? `${actualTarget} <small class="text-muted">(${totalPeserta} peserta × ${item.nilai_target})</small>`
                                : `${item.nilai_target}`;
                        }
                        ...
                        if (item.jabatan) {
                            ...
                            if (jabatanList.length > 0) {
                                jabatanDisplay = jabatanList.length === 1 
                                    ? jabatanList[0] 
                                    : jabatanList.map(j => String(j).substring(0, 4) + '...').join(', ');
                            }
                        }
                        ...
                        if (item.tipe_target === 'angka') {
                            isTargetReached = (item.manual_value ?? 0) >= actualTarget;
                        } else if (item.tipe_target === 'rupiah') {
                            isTargetReached = progress >= actualTarget;
                        } else {
            ...
            function applyFilterAndRender() {
                ...
                currentFilteredData = allTargetData.filter(item => {
                    const divisiStr = Array.isArray(item.divisi) ? item.divisi.join(' ') : String(item.divisi || '');
                    const pembuatStr = String(item.pembuat || '');
                    const judulStr = String(item.judul || '');
                    const jabatanStr = String(item.jabatanDisplay || '');
                    const statusStr = String(item.statusText || '');

                    return (
                        judulStr.toLowerCase().includes(searchVal) ||
                        pembuatStr.toLowerCase().includes(searchVal) ||
                        divisiStr.toLowerCase().includes(searchVal) ||
                        jabatanStr.toLowerCase().includes(searchVal) ||
                        statusStr.toLowerCase().includes(searchVal)
                    );
                });
