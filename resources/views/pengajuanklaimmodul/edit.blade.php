            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body" id="card">
                        <a href="{{ route('pengajuanklaimmodul.show', $klaimModul->id) }}" class="btn click-primary my-2">
                            <img src="{{ asset('icon/arrow-left.svg') }}" width="20px"> Back
                        </a>
                        <h5 class="card-title text-center mb-4">{{ __('Edit Klaim Modul') }}</h5>
                        <form method="POST" action="{{ route('pengajuanklaimmodul.update', $klaimModul->id) }}">
                            @csrf @method('PUT')
                            <div class="row mb-3">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="title" type="text" class="form-control @error('title') is-invalid @enderror"
                                        name="title" value="{{ old('title', $klaimModul->module->title) }}" required>
                                ...
                                <div class="col-md-6">
                                    <select id="category" name="category"
                                        ...
                                        @foreach($categories as $cat)
                                            @if($cat)
                                                <option value="{{ $cat }}" {{ old('category', $klaimModul->module->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    @php $selectedInstructors = old('instructors', $klaimModul->module->instructors->pluck('id')->toArray()); @endphp
                                    <select name="instructors[]" id="instructors"
                                        class="form-select select2 @error('instructors') is-invalid @enderror" multiple
                                        required>
                                        @foreach($instructors as $instructor)
                                            <option value="{{ $instructor->id }}" {{ in_array($instructor->id, $selectedInstructors) ? 'selected' : '' }}>
                                                {{ $instructor->karyawan->nama_lengkap ?? $instructor->name ?? $instructor->username }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <textarea id="description"
                                        ...
                                        rows="4">{{ old('description', $klaimModul->module->description) }}</textarea>
                            ...
                            <div class="row mb-3">
                                <label for="link" class="col-md-4 col-form-label text-md-start">{{ __('Link Modul') }} <span
                                        class="text-danger">*</span></label>
                                <div class="col-md-6">
                                    @if($klaimModul->module->link)
                                        <div class="mb-2"><small class="text-muted">🔗 Link saat ini: <a
                                                    href="{{ $klaimModul->module->link }}"
                                                    target="_blank">{{ Str::limit($klaimModul->module->link, 50) }}</a></small>
                                        </div>
                                    @endif
                                    <input id="link" type="url" class="form-control @error('link') is-invalid @enderror"
                                        name="link" value="{{ old('link', $klaimModul->module->link) }}"
                            ...
                            <div class="row mb-0">
                                <div class="col-md-6 offset-md-4">
                                    <button type="submit" class="btn click-primary">{{ __('Simpan Perubahan') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
