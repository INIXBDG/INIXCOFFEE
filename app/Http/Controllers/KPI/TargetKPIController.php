...
class TargetKPIController extends Controller
{
    ...
    public function getAssistantRoutesByJabatan(Request $request)
    {
        ...
        $kombinasiIT = ['programmer', 'tim digital', 'technical support', 'project administrator & business support'];
    ...
    private function calculateLeadsProject($item, $personId) {
        ...
        $totalLeads = LeadProject::where('status', '!=', 'lost')
            ->join('projects', 'lead_projects.id', '=', 'projects.lead_id')
            ->whereYear('projects.tanggal_awal', $tahun)->count();

        $progress = ($target * $totalLeads) / 100;
    ...
    private function calculatePresentaseKinerjaInstruktur($item, $personId)
    {
        ...
        if ($personId !== null) {
            ...
            $targetJam = $jumlahInstruktur * $targetJamPerOrang;
        }
    ...
    private function calculateMeningkatkanRevenuePerusahaan($item, $personId)
    {
        ...
        $Approval = ApprovalPendapatan::whereYear('created_at', $tahun)
            ->get();

        foreach ($Approval as $p) {
            $progress += $p->total_penjualan_bersih;
        }
    ...
    private function calculateLeadsProjectDetail($itemDetail)
    {
        ...
        // QUERY DATA LEADS PROJECT
        $query = LeadProject::where('status', '!=', 'lost')
            ->join('projects', 'lead_projects.id', '=', 'projects.lead_id')
            ->whereYear('projects.tanggal_awal', $tahun);

        $sales = $query
            ->select('projects.tanggal_awal', DB::raw('COUNT(*) as total'))
            ->groupBy('projects.tanggal_awal')
            ->get();
        ...
        $triwulanData = [];
        for ($i = 1; $i <= 4; $i++) {
            $triwulanData['Triwulan_' . $i] = (int) $triwulanDataTemp[$i];
        }

        $progressGlobal = $totalLeads;

        $gap = $progressGlobal - $target;

        $above = $progressGlobal;
        $below = $progressGlobal - $target;

        $monthlyProgress = [];
        foreach ($monthlyData as $month => $value) {
            $monthlyProgress[$month] = $target > 0
                ? (float) number_format(($value / $target) * 100, 1, '.', '')
                : 0;
        }
        ...
        foreach ($dailyBreakdownPerMonth as $month => $days) {
            foreach ($days as $day => $value) {
                $dailyProgressPerMonth[$month][$day] = $target > 0
                    ? (float) number_format(($value / $target) * 100, 1, '.', '')
                    : 0;
            }
        }
    ...
    private function calculatePresentaseKinerjaInstrukturDetail($itemDetail, $personId)
    {
        ...
        $progress = round(min(100, ($totalJamMengajar / $targetJam) * 100), 1);
        $gap = round($progress - 100, 1);

        $above = $totalJamMengajar;
        $below = $personId ? 0 : max(0, $targetJam - $totalJamMengajar);
        ...
        foreach ($dailyValues as $dateStr => $jam) {
            ...
            $monthly[$m] = ($monthly[$m] ?? 0) + $jam;
            $dailyPerMonth[$m][$dateStr] = $jam;
        }

        foreach ($monthly as $month => $totalJam) {
            $monthlyProgress[$month] = $targetJamPerOrang > 0
                ? round(($totalJam / $targetJamPerOrang) * 100, 1)
                : 0;
        }

        foreach ($dailyPerMonth as $month => $days) {
            foreach ($days as $d => $val) {
                $dailyProgress[$month][$d] = $targetJamPerOrang > 0
                    ? round(($val / $targetJamPerOrang) * 100, 1)
                    : 0;
            }
        }
    ...
    private function calculateMeningkatkanRevenuePerusahaanDetail($itemDetail)
    {
        ...
        $peluangs = ApprovalPendapatan::with(['rkm.perhitunganNetSales'])
            ->whereYear('created_at', $tahun)
            ->get();
        ...
        foreach ($peluangs as $p) {
            ...
            $dailyBreakdownPerMonth[$monthKey][$dateKey] += $p->total_penjualan_bersih;
        }
