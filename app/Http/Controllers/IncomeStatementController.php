...
class IncomeStatementController extends Controller
{
    ...
    public function laporan(Request $request)
    {
        ...
        // ==========================================
        $fc_inventaris_keys = collect(range(1, 6))->map(fn ($i) => 'fc_'.$i)->toArray();
        $biaya_inventaris = $transactions->whereIn('item_code', $fc_inventaris_keys)->sum('amount');

        $fc_gaji_keys = collect(range(7, 19))->map(fn ($i) => 'fc_'.$i)->toArray();
        $biaya_tunjangan_karyawan = $transactions->whereIn('item_code', $fc_gaji_keys)->sum('amount');

        $fc_spj_keys = collect(range(20, 40))->map(fn ($i) => 'fc_'.$i)->toArray();
        $biaya_spj = $transactions->whereIn('item_code', $fc_spj_keys)->sum('amount');

        $fc_operasional_keys = collect(range(41, 87))->map(fn ($i) => 'fc_'.$i)->toArray();
